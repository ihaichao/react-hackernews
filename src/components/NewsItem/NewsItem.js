...
export default class NewsItem extends Component {
	...
	render () {
		...
		return (
			<div className="item">
				<span className="index">{(item.page - 1) * item.itemsPerPage + item.index + 1}.</span>
				<p>
					<a className="title" target="_blank" rel="noopener noreferrer">{item.title}</a>
					{item.url && <span className="domain"> ({this.getDomain(item.url)})</span>}
				</p>
				<p className="subtext">
					<span>
						{item.score} points by
						<a className="by" href="//news.ycombinator.com/user?id={item.by}" target="_blank" rel="noopener noreferrer">{item.by}</a>
					</span>
					{this.getTimeFromNow(item.time)} ago
